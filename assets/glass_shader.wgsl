...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let border_width = 0.05; // How thick the border effect is (0.0 to 1.0 relative to dist_from_center)
    ...
    let border_highlight_color = vec3<f32>(1.0, 0.0, 1.0);
    // Strength of the whitening effect at the border
    let border_highlight_strength = 0.4; 
